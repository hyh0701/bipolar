...
import com.hyh.jizhang.dao.ClassificationDao;
...
public class AlterAccountFrameController {
    ...
    public void setTableData(TableData tableData) {
        ...
        if(outputRadioButton.isSelected()){
            List<Classification> classificationList=new ClassificationDao().selectByType("支出");
            String[] classficationNames=new String[classificationList.size()];
            //将查询到的分类名装到一维数组
            for (int i = 0; i < classificationList.size(); i++) {
                classficationNames[i]=classificationList.get(i).getcName();
            }
            classificationComboBox.setItems(FXCollections.observableArrayList((List) Arrays.asList(classficationNames)));
        }
        ...
        if(tableData.getType().equals("支出")){
            ...
            List outputList=FXCollections.observableArrayList((List) Arrays.asList(classficationNames));
            for (int i = 0; i < outputList.size(); i++) {
                if(str.equals(outputList.get(i))){
                    index=i;
                }
        ...
        if(tableData.getType().equals("收入")){
            ...
            List inputList=FXCollections.observableArrayList((List) Arrays.asList(classficationNames));
            for (int i = 0; i < inputList.size(); i++) {
                if(str.equals(inputList.get(i))){
                    index=i;
                }
